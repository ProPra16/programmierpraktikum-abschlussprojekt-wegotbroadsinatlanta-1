...
import TDDT.Main;
import TDDT.Fensterdesigns.FinalWindow;
...
public class TDDTController implements Initializable{
    ...
    public void run(){
        ...
        }catch(Exception e){DEBUG.out("Error in Compilation");Main.self.TDDTStatusBar.output.setText("Error in Compilation");e.printStackTrace();}
        errorcounter.setText("Fehler in Tests: " + result.numberOfFailedTest);
        Main.self.TDDTStatusBar.output.setText(result.outmessage);
        if(result.hasErrors){ Main.self.TDDTStatusBar.output.setTextFill(Color.web("#FF6B68"));}else{Main.self.TDDTStatusBar.output.setTextFill(Color.web("#e3e3e3"));}
    }
